...
9.If buying each product generates point for eg. 5rs=2 zomato points 
and each product has different purchasing points for eg. for p1 5rs=1 zomato point,
for p2 10rs=5 zomato point and p3 5rs=1 zomato point 2rs=1 zomato point,
...
(select d.*,case when product_id=1 then 5 when product_id=2 then 2 
 when product_id=3 then 5 else o end as points from
 (select c.userid, c.product_id,sum(price)amt from
 (select a.*, b.price from sales a.inner join product b on a.product_id=b.product_id)c
 group by userid, product_id)d)e)f group by userid;
 
select* from
(select* , rnk()over(order by total_point_earned desc)rnk from
...
(select d.*,case when product_id=1 then 5 when product_id=2 then 2 
 ...
 (select c.userid, c.product_id,sum(price)amt from
 (select a.*, b.price from sales a.inner join product b on a.product_id=b.product_id)c
 group by userid, product_id)d)e)f group by product_id)f)rnk1;
